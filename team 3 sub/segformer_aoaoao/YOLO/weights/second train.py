"""
BD100K 训练 YOLO11s-P2 终极修正版（支持断点续训）
- 完美适配你的 JSON 格式（frames.objects）
- 自动跳过已有有效标签，强制重新生成空标签
- 200 轮训练，每轮显示精度，覆盖保存最新模型
- 新增：从第95轮检查点继续训练，直至200轮
"""
...
# ======================= 用户设置（只改这里） =======================
DATA_PATH = "D:/combined_15cls"          # 你的数据集根目录

CONVERT_JSON = True              # 保持 True，自动转换；如果之前已经成功转换，可设 False 加快启动
JSON_LABEL_DIR = "D:/combined_15cls/labels/train"
JSON_IMG_DIR = "D:/combined_15cls/images/train"
OUTPUT_LABEL_DIR = "D:/combined_15cls/labels/train"
...
RESUME = True                    # 是否从检查点继续训练
RESUME_CHECKPOINT = "runs/detect/train-5/weights/last.pt"  # 第95轮保存的模型路径
# =============================================================
...
def convert_json_folder_to_yolo(json_dir, img_dir, out_dir):
    """
    智能转换：处理 frames.objects 结构，跳过 poly2d，跳过已有有效标签，
    但如果旧 .txt 为空则重新生成。
    """
...
def main():
    ...
    if RESUME:
        if not os.path.exists(RESUME_CHECKPOINT):
            raise FileNotFoundError(f"找不到检查点文件: {RESUME_CHECKPOINT}，请确认路径！")
        print(f"从检查点恢复训练: {RESUME_CHECKPOINT}")
        # 从检查点加载模型（自动继承结构和权重）
        model = YOLO(RESUME_CHECKPOINT)
        # 继续训练时也要注册回调，确保后续每轮继续保存
        model.add_callback("on_epoch_end", save_epoch_model)

        # 使用 resume=True，它会自动从检查点中记录的最新 epoch 继续
        # 总轮数仍是 EPOCHS=200，实际运行剩余的轮数
        model.train(
            resume=True,            # 核心：继续训练
            data=os.path.join(DATA_PATH, "data.yaml"),
            epochs=EPOCHS,          # 总目标轮数，内部会从断点开始
            imgsz=IMG_SIZE,
            batch=BATCH_SIZE,
            device=DEVICE,
            workers=WORKERS,
