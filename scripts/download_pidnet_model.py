...
def _validate_model_directory(config: dict[str, object], directory: Path) -> None:
    ...
    for filename, (expected_size, expected_sha) in required.items():
        ...
        if not path.is_file():
            raise RuntimeError(f"missing PIDNet asset file: {path.relative_to(ROOT)}")
    ...
    labels = (directory / str(model["labels_file"])).read_text(encoding="utf-8").splitlines()
    if not labels or labels[0] != "road":
        raise RuntimeError("PIDNet labels must contain road at class index 0")
...
def main() -> int:
    ...
    if all(path.is_file() for path in missing):
        _validate_model_directory(config, directory)
        print(f"PIDNet checkpoint verification passed: {directory.relative_to(ROOT)}")
        return 0
